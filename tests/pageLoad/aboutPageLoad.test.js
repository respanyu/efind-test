...
const TEST_NAME = "About Page Load";
const ABOUT_PATH = "/about";

async function testAboutPageLoad() {
  ...
  try {
    ...
    await driver.get(config.baseUrl + ABOUT_PATH);
    checks.push("About page URL opened successfully");

    pageTitle = await driver.getTitle();
    if (!pageTitle || pageTitle.trim() === "") {
      throw new Error("About page title is missing");
    }
    checks.push("About page title is present");
    ...
    const screenshotPath = await takeScreenshot(driver, "about_page_load");
...
module.exports = testAboutPageLoad;
